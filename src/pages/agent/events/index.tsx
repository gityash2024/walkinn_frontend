// src/pages/agent/events/index.tsx
...
const AgentEventList = () => {
  ...
  const handleEditEvent = (event: Event) => {
    // Only allow editing of events created by this agent
    if (event.organizer.id === 'current-agent-id') {
  ...
  const handleFormSubmit = async (values: any) => {
    try {
      if (selectedEvent) {
        ...
        const updatedEvent = { ...selectedEvent, ...values, isPublished: false };
  ...
  const renderEvents = () => {
    ...
    const filteredEvents = events.filter(event => 
      event.organizer.id === 'current-agent-id' || event.isPublished
    );
                ...
                <tr key={event.id} className="border-b border-gray-700">
                  ...
                  <td className="p-4">
                    <div className="flex space-x-2">
                      {event.organizer.id === 'current-agent-id' && (
                        <button
                      ...
                      {!event.isPublished && event.organizer.id === 'current-agent-id' && (
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      ...
      {/* Notice for unpublished events */}
      <div className="mt-8 p-4 bg-yellow-900 bg-opacity-20 rounded-lg">
        <div className="flex items-center text-yellow-400 mb-2">
          <AlertCircle className="w-5 h-5 mr-2" />
          <h3 className="font-bold">Important Notice</h3>
        </div>
        <p className="text-yellow-200">
          All created events require admin approval before being published. Once approved, they will be visible to users.
        </p>
      </div>
    </div>
  );
